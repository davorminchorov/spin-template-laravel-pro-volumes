...
SPIN_FRAMEWORK_ARGS=("${SPIN_FRAMEWORK_ARGS[@]:-$@}")
...
if [ "$SPIN_ACTION" == "new" ]; then
  ...
  laravel_project_directory=${SPIN_FRAMEWORK_ARGS[0]:-laravel}
...
new(){
  ...
  docker run --rm -v "$(pwd):/var/www/html" --user "${SPIN_USER_ID}:${SPIN_GROUP_ID}" -e COMPOSER_CACHE_DIR=/dev/null -e "SHOW_WELCOME_MESSAGE=false" "$SPIN_PHP_DOCKER_IMAGE" composer --no-cache create-project laravel/laravel "${SPIN_FRAMEWORK_ARGS[@]}"
...
init(){
  ...
  docker run --rm -v "$SPIN_PROJECT_DIRECTORY:/var/www/html" --user "${SPIN_USER_ID}:${SPIN_GROUP_ID}" -e COMPOSER_CACHE_DIR=/dev/null -e "SHOW_WELCOME_MESSAGE=false" "$SPIN_PHP_DOCKER_IMAGE" composer --verbose --working-dir=/var/www/html/ require serversideup/spin --dev
}
